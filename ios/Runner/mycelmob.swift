...
fileprivate extension RustBuffer {
    ...
    static func from(_ ptr: UnsafeBufferPointer<UInt8>) -> RustBuffer {
        try! rustCall { ffi_mycelmob_rustbuffer_from_bytes(ForeignBytes(bufferPointer: ptr), $0) }
    }

    // Frees the buffer in place.
    // The buffer must not be used after this is called.
    func deallocate() {
        try! rustCall { ffi_mycelmob_rustbuffer_free(self, $0) }
    }
}
...
public func addressFromSecretKey(data: Data) -> String {
    return try!  FfiConverterString.lift(try! rustCall() {
    uniffi_mycelmob_fn_func_address_from_secret_key(
...
public func generateSecretKey() -> Data {
    ...
    uniffi_mycelmob_fn_func_generate_secret_key($0
    )
})
}
public func getPeerStatus() -> [String] {
    return try!  FfiConverterSequenceString.lift(try! rustCall() {
    uniffi_mycelmob_fn_func_get_peer_status($0
    )
})
}
public func helloInt() -> Int32 {
    return try!  FfiConverterInt32.lift(try! rustCall() {
    uniffi_mycelmob_fn_func_hello_int($0
    )
})
}
public func helloMycelios() -> String {
    return try!  FfiConverterString.lift(try! rustCall() {
    uniffi_mycelmob_fn_func_hello_mycelios($0
    )
})
}
...
private var initializationResult: InitializationResult = {
    ...
    let scaffolding_contract_version = ffi_mycelmob_uniffi_contract_version()
    if bindings_contract_version != scaffolding_contract_version {
        return InitializationResult.contractVersionMismatch
    }
    if (uniffi_mycelmob_checksum_func_address_from_secret_key() != 39059) {
        return InitializationResult.apiChecksumMismatch
    }
    if (uniffi_mycelmob_checksum_func_generate_secret_key() != 63601) {
        return InitializationResult.apiChecksumMismatch
    }
    if (uniffi_mycelmob_checksum_func_get_peer_status() != 1198) {
        return InitializationResult.apiChecksumMismatch
    }
    if (uniffi_mycelmob_checksum_func_hello_int() != 31063) {
        return InitializationResult.apiChecksumMismatch
    }
